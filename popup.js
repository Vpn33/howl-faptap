...
function loadConfig() {
  ...
  if (cachedConfig) {
    try {
      ...
      // 设置界面值
      document.getElementById('serverAddress').value = config.serverAddress || 'http://127.0.0.1';
      document.getElementById('syncDelay').value = config.syncDelay !== undefined ? config.syncDelay : '0';
      console.log('从本地缓存加载配置:', config);
      return; // 已加载缓存，直接返回
    } catch (error) {
      console.log('解析缓存配置失败:', error);
  ...
  chrome.runtime.sendMessage({ action: 'get_config' }, (response) => {
    try {
      ...
      // 设置默认值
      const serverAddress = config.serverAddress || config.serverIp ? `http://${config.serverIp}:4695` : 'http://127.0.0.1';
  ...
  const sendSeekMessage = () => {
    chrome.runtime.sendMessage({ 
      ...
      try {
        ...
        console.log('跳转播放位置响应处理失败:', error);
...
document.addEventListener('DOMContentLoaded', () => {
  loadConfig();
  loadCachedFunscript(); // 加载缓存的funscript信息
